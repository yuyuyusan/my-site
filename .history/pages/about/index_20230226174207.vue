        ...
        <div class="content">
          <div v-show="tab === 1" class="contentItem">
            <div class="wrap">
              <p>
                <span>SKILLS</span>
                現在、経験年数1年10ヶ月ですが就業時から、社内に先輩コーダー、エンジニアなどがいないため、今もほとんど独学です。そのため最初は、時間効率、技術の順序等も悪く、2022年6月頃から技術が伸びている気がしていて、更に2023年1月からもっと伸びている気がしています。実質1年弱位の感覚です。<br>
                レスポンシブデザインはPCのデザインが基本上がってきて、自分で考えて作成しているので対応可能です。<br>
    ...
    return {
      ...
      skill: [
        ...
        },
        {
          ...
          level: "5",
          num: "3",
          desc: '現職でも使用できる人がいないので、自分の評価が難しいですが、中級レベルくらいと思っています。これからもどんどん突き詰めます。開発などフルスクラッチでの経験はありません。SPAサイトが自分のできる限りのことを書いています。会社の業務ではほとんど使用しないですが、使用する場面では、あえて新しい方法での、実装を心がけています。自分のサイトで、少し開発的なことを実装してみようと思います。'
        },
        {
          icon: "../icon-php.svg",
          title: "PHP",
          level: "4",
          num: "4",
          desc: "JavaScriptと同じく、基本の形は抑えていますが、経験業務はWordPressと修正案件での自作フォーム(smartyを使用)のみです。ほとんどWordpressがメインです。自身のSPAサイトで素のPHPでフォームを作成し、データベースと接続をしました。"
        },
        {
        ...
        },
        {
          ...
          level: "7",
          num: "7",
          desc: "テンプレート階層を理解して、オリジナルテーマを作成でき、サーバー移転、function.phpの記述ができます。1回だけTCDのテーマから制作がありましたが、オリジナルテーマのほうがやりやすいです。機能少なめなECサイトの制作は業務で経験しました。プラグインは作り方は知っていますが、自分で作成して業務で使用はしたことはありません。"
        },
        {
          icon: "../icon-vue.svg",
          title: "Vue.js",
        ...
        },
        {
          ...
          level: "1",
          num: "11",
          desc: "色々調べているうちに、JavaScriptを極めるならnode.jsも触って行かなければ行けないのかなと。優先順位的にはtsくらいで、少し低めではありますが、いつの日か習得します。今はexpressかなとぼんやり考えています。"
        },
        {
          icon: "../icon-git.svg",
          title: "git",
          level: "3",
          num: "12",
          desc: "会社でgitを使用していないため、共同で使用したことがありません。githubで個人で管理していて、add,commit,pushなど基本操作は理解しています。"
        },
        {
          icon: "../icon-mysql.svg",
          title: "MySql",
          level: "3",
          num: "13",
          desc: "WordPressの構造だけほんの少し知っている状態から、少し知識を増やした程度です。自身で作成している、SPAサイトでフォームから送信されたデータを接続、格納までは可能です。phpmyadminを利用しており、基本構文の意味などは理解しています。"
        },
        {
          icon: "../icon-pc.svg",
          title: "その他",
